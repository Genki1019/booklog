...
export const getBooks = async (req, res) => {
    try {
        ...
        const whereClause = {};
        if (title)
            whereClause.title = { contains: String(title) };
        if (author)
    ...
    try {
        ...
        if (existingBook.imageUrl && existingBook.imageUrl.startsWith(`${BASE_URL}/uploads/`) && existingBook.imageUrl !== updatedBook.imageUrl) {
            ...
            fs.unlink(oldImagePath, (err) => {
                if (err)
                    console.warn(messages.errors.imageDeleteError(oldImagePath), err);
                else
                    console.log(messages.success.imageDeleted(oldImagePath));
            });
        }
        res.json({
            id: updatedBook.id,
